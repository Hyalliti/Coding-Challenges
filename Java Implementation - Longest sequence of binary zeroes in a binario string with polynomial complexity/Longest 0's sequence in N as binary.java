...
class Main {
  ...
  public static void solution(int N){
    ...
    // Entero Mayor a 9, corre el codigo
    else{
      String binario = "";
          int r=0;
      ...
      for (int i = 0; i <  longitud ; i++){
        ...
        if(binario.charAt(binario.length()-1) == '0' & (binario.split(cadena_zero[gap-1],-1).length - 1) <= 1) {
          do{
            ...
            System.out.println("Index to be reduced is:"+ index);
            gap = gap-checker;
            checker += 1;
            System.out.println("Reduced Binary Gap is: T1 "+ binario);
            gap = gap-1;
            System.out.println("Reduced Binary Gap is: T1 "+ gap);
          }while(matcher.find() == true & binario.charAt(binario.length()-1) == '0' & gap != 0);
        }
        else{
          System.out.println("Binary Gap is: T2 "+ gap);
        }
      }
    }
}
